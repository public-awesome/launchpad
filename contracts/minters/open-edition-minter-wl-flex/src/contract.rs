...
use cosmwasm_std::{
    ...
    Event, MessageInfo, Order, Reply, Response, StdError, StdResult, SubMsg, Timestamp, WasmMsg,
};
...
pub fn instantiate(
    ...
    // Create collection_info_extension before SubMsg
    let collection_info_extension: Option<CollectionExtensionMsg<RoyaltyInfoResponse>> =
        Some(CollectionExtensionMsg {
            description: Some(collection_info.description.clone()),
            image: Some(collection_info.image.clone()),
            ...
            royalty_info: collection_info
                .royalty_info
                .clone()
                .map(|r| RoyaltyInfoResponse {
...
fn _execute_mint(
    ...
    let network_fee = mint_price.amount.mul_floor(mint_fee);
...
pub fn reply(deps: DepsMut, _env: Env, msg: Reply) -> Result<Response, ContractError> {
    ...
    // Extract data from msg_responses first
    let data = msg
        .result
        .into_result()
