...
use sg4::{MinterConfigResponse, Status, StatusResponse, SudoMsg};
...
fn query_config(deps: Deps) -> StdResult<MinterConfigResponse<ConfigExtension>> {
    let config = CONFIG.load(deps.storage)?;
    let sg721_address = SG721_ADDRESS.load(deps.storage)?;

    Ok(MinterConfigResponse {
        config,
        collection_address: sg721_address.to_string(),
    })
}
