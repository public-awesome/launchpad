use bech32::{convert_bits, decode, encode, Error, ToBase32, Variant};
...
fn convert_and_encode(hrp: String, data: Vec<u8>) -> Result<String, Error> {
    ...
    encode(&hrp, converted.to_base32(), Variant::Bech32)
}

#[test]
fn to_stars() {
    let decoded = decode_and_convert("cosmos1p655nr52wepstj39jr3skwv6nkmdrfka99j802").unwrap();
    let addr = convert_and_encode("stars".to_string(), decoded.1).unwrap();
    assert_eq!(
        addr,
        "stars1p655nr52wepstj39jr3skwv6nkmdrfka3e96ym".to_string()
    );
}
