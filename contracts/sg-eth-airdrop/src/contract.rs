...
fn claim_airdrop(
    ...
    let valid_eth_signature =
        query_verify_ethereum_text(deps.as_ref(), &config.claim_msg_plaintext, &eth_sig_hex, &eth_address).unwrap();
    
    Ok(Response::new()
        .add_attribute("amount", amount.to_string())
        .add_attribute("result", valid_eth_signature.verifies.to_string())
