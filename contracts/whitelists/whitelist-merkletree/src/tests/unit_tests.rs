...
mod tests {
    ...
    fn setup_contract(deps: DepsMut, merkle_root: Option<String>) {
        ...
        let info = message_info(&admin, &[coin(CREATION_AMOUNT, NATIVE_DENOM)]);
    ...
    fn query_membership() {
        ...
        let user1 = "stars1ye63jpm474yfrq02nyplrspyw75y82tptsls9t";
        let res = query_has_member(deps.as_ref(), user1.to_string(), hash_strings).unwrap();
        assert!(res.has_member);

        let user2 = "stars130dxx3nr2ste4fwsum57k3en60wqd76m9pvpsy";
        let proof = vec![
        ...
        let user3 = "stars16epdu6c7h8apxrnuu06yzfxflrede0mtu4qqz4";
        let proof = vec![
        ...
        let _ = query_has_member(deps.as_ref(), user1.to_string(), proof).unwrap_err();
    }
}
