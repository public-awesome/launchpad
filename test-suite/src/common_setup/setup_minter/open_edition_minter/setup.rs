...
use crate::common_setup::msg::{MinterCollectionResponse, OpenEditionMinterInstantiateParams, OpenEditionMinterSetupParams};
...
use crate::common_setup::setup_minter::open_edition_minter::mock_params::{mock_create_minter, mock_init_minter_extension, mock_params_proper};
...
pub fn build_init_msg(
    ...
    match init_msg {
        ...
        None => mock_init_minter_extension(start_time, end_time, per_address_limit_minter, mint_price, nft_data, payment_address)
    }
}

// Upload contract code and instantiate open edition minter contract
pub fn setup_open_edition_minter_contract(setup_params: OpenEditionMinterSetupParams) -> MinterCollectionResponse {
    ...
    let factory_addr = router
        .instantiate_contract(
            factory_code_id,
            minter_admin.clone(),
            &open_edition_factory::msg::InstantiateMsg { params: params.clone() },
            &[],
            "factory",
            None,
        );
...
pub fn configure_open_edition_minter(
    ...
    for (index, collection_param) in collection_params_vec.iter().enumerate() {
        let setup_params: OpenEditionMinterSetupParams = OpenEditionMinterSetupParams {
            ...
            nft_data: minter_instantiate_params_vec[index].nft_data.to_owned().unwrap(),
            per_address_limit: minter_instantiate_params_vec[index].per_address_limit.to_owned().unwrap(),
