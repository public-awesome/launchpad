...
use crate::sg_eth_airdrop::constants::claim_constants::{stargaze_wallet_01, CONFIG_PLAINTEXT};
...
fn test_set_minter_contract_success() {
    ...
    let contract_admin = creator.clone();
    ...
    let result: cosmwasm_std::Addr = app
        .wrap()
...
fn test_claim_added_to_minter_whitelist() {
    ...
    let stargaze_wallet_01 = stargaze_wallet_01();
    update_admin_for_whitelist(&mut app, creator, airdrop_contract.clone(), whitelist_addr);
    send_funds_to_address(&mut app, &stargaze_wallet_01.to_string(), MINT_PRICE);
