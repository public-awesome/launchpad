...
#[test]
fn test_factory_execute_whitelist_operations() {
    use vending_factory::msg::{ExecuteMsg, IsContractWhitelistedResponse, QueryMsg as FactoryQueryMsg, WhitelistedContractsResponse};
    ...
    // Test execute-based add to whitelist (open operation)
    let add_msg = ExecuteMsg::AddContractToWhitelist {
        address: contract_addr.to_string(),
    };
    let res = router.execute_contract(creator.clone(), factory.clone(), &add_msg, &[]);
    assert!(res.is_ok(), "Should be able to add contract via execute");
    ...
    // Test execute-based remove from whitelist
    let remove_msg = ExecuteMsg::RemoveContractFromWhitelist {
        address: contract_addr.to_string(),
    };
    let res = router.execute_contract(creator, factory.clone(), &remove_msg, &[]);
    assert!(res.is_ok(), "Should be able to remove contract via execute");
...
#[test]
fn test_factory_execute_batch_whitelist_update() {
    use vending_factory::msg::{ExecuteMsg, IsContractWhitelistedResponse, QueryMsg as FactoryQueryMsg};
    ...
    router.execute_contract(creator.clone(), factory.clone(), &add_msg, &[]).unwrap();
    ...
    let res = router.execute_contract(creator, factory.clone(), &batch_msg, &[]);
    assert!(res.is_ok(), "Batch update should succeed");
    ...
    let res3: IsContractWhitelistedResponse = router
        .wrap()
        .query_wasm_smart(factory, &query_msg3)
        .unwrap();
    assert!(res3.is_whitelisted, "Contract3 should be added");
}

#[test]
fn test_instantiate_whitelist_is_working() {
    // Note: This is a validation test to ensure the instantiate whitelist feature
    // is properly implemented. The existing template uses initial_whitelist: None,
    // so we validate that no contracts are whitelisted by default.
    use vending_factory::msg::{IsContractWhitelistedResponse, QueryMsg as FactoryQueryMsg};

    let vt = vending_minter_template_with_code_ids_template(1);
    let (router, _, _) = (vt.router, vt.accts.creator, vt.accts.buyer);
    let factory = vt.collection_response_vec[0].factory.clone().unwrap();

    let contract_addr = "contract1234567890abcdefghijklmnopqrstuvwxyz1234567890abcdefghij";

    // Check that contract is not whitelisted (since factory was created with None)
    let query_msg = FactoryQueryMsg::IsContractWhitelisted {
        address: contract_addr.to_string(),
    };
    let res: IsContractWhitelistedResponse = router
        ...
        .query_wasm_smart(factory, &query_msg)
        .unwrap();
    assert!(!res.is_whitelisted, "Contract should not be whitelisted when factory created with initial_whitelist: None");
}
