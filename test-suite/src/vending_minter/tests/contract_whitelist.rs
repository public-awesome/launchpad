...
use vending_minter::msg::{IsContractWhitelistedResponse, QueryMsg, WhitelistedContractsResponse};
...
#[test]
fn test_admin_can_add_contract_to_whitelist() {
    ...
    // Admin should be able to add contract to whitelist
    let add_msg = ExecuteMsg::AddContractToWhitelist {
        address: contract_addr.to_string(),
    };

    let res = router.execute_contract(creator, minter_addr.clone(), &add_msg, &[]);
    assert!(res.is_ok(), "Admin should be able to add contract to whitelist");

    // Check that the contract is now whitelisted
    let query_msg = QueryMsg::IsContractWhitelisted {
        address: contract_addr.to_string(),
    };
    let res: IsContractWhitelistedResponse = router
        .wrap()
        .query_wasm_smart(minter_addr, &query_msg)
        .unwrap();
...
#[test]
fn test_non_admin_cannot_add_contract_to_whitelist() {
    let vt = vending_minter_template(1);
    ...
    // Non-admin should not be able to add contract to whitelist
    let add_msg = ExecuteMsg::AddContractToWhitelist {
        address: contract_addr.to_string(),
    };

    let res = router.execute_contract(buyer, minter_addr, &add_msg, &[]);
    assert!(res.is_err(), "Non-admin should not be able to add to whitelist");
...
fn test_whitelisted_contract_can_mint() {
    ...
    // Admin adds contract to whitelist
    let add_msg = ExecuteMsg::AddContractToWhitelist {
        address: contract_addr.to_string(),
    };
    let res = router.execute_contract(creator, minter_addr.clone(), &add_msg, &[]);
...
#[test]
fn test_remove_contract_from_whitelist() {
    let vt = vending_minter_template(1);
    let (mut router, creator, _buyer) = (vt.router, vt.accts.creator, vt.accts.buyer);
    let minter_addr = vt.collection_response_vec[0].minter.clone().unwrap();
    ...
    router
        .execute_contract(creator.clone(), minter_addr.clone(), &add_msg, &[])
        .unwrap();
    ...
    let res: IsContractWhitelistedResponse = router
        .wrap()
        .query_wasm_smart(minter_addr.clone(), &query_msg)
        .unwrap();
    assert!(res.is_whitelisted);

    // Remove from whitelist
    let remove_msg = ExecuteMsg::RemoveContractFromWhitelist {
        address: contract_addr.to_string(),
    };
    let res = router.execute_contract(creator, minter_addr.clone(), &remove_msg, &[]);
    assert!(res.is_ok(), "Admin should be able to remove from whitelist");
...
#[test]
fn test_batch_update_contract_whitelist() {
    let vt = vending_minter_template(1);
    let (mut router, creator, _buyer) = (vt.router, vt.accts.creator, vt.accts.buyer);
    let minter_addr = vt.collection_response_vec[0].minter.clone().unwrap();
    ...
    // Add contract1 to whitelist first
    let add_msg = ExecuteMsg::AddContractToWhitelist {
    ...
    let batch_msg = ExecuteMsg::UpdateContractWhitelist {
        add: vec![contract2.to_string(), contract3.to_string()],
        remove: vec![contract1.to_string()],
    };

    let res = router.execute_contract(creator, minter_addr.clone(), &batch_msg, &[]);
    assert!(res.is_ok(), "Admin should be able to batch update whitelist");

    // Check results
    let query_msg1 = QueryMsg::IsContractWhitelisted {
        address: contract1.to_string(),
    };
    let res1: IsContractWhitelistedResponse = router
        .wrap()
        .query_wasm_smart(minter_addr.clone(), &query_msg1)
        .unwrap();
    assert!(!res1.is_whitelisted, "Contract1 should be removed");

    let query_msg2 = QueryMsg::IsContractWhitelisted {
        address: contract2.to_string(),
    };
    let res2: IsContractWhitelistedResponse = router
        .wrap()
        .query_wasm_smart(minter_addr.clone(), &query_msg2)
        .unwrap();
    assert!(res2.is_whitelisted, "Contract2 should be added");

    let query_msg3 = QueryMsg::IsContractWhitelisted {
        address: contract3.to_string(),
    };
    let res3: IsContractWhitelistedResponse = router
        .wrap()
        .query_wasm_smart(minter_addr, &query_msg3)
        .unwrap();
    assert!(res3.is_whitelisted, "Contract3 should be added");
}

#[test]
fn test_query_whitelisted_contracts() {
    let vt = vending_minter_template(1);
    let (mut router, creator, _buyer) = (vt.router, vt.accts.creator, vt.accts.buyer);
    let minter_addr = vt.collection_response_vec[0].minter.clone().unwrap();
    ...
    // Add both contracts to whitelist
    let batch_msg = ExecuteMsg::UpdateContractWhitelist {
        add: vec![contract1.to_string(), contract2.to_string()],
        remove: vec![],
    };
    router
        .execute_contract(creator, minter_addr.clone(), &batch_msg, &[])
        .unwrap();

    // Query all whitelisted contracts
    let query_msg = QueryMsg::WhitelistedContracts {
        start_after: None,
        limit: None,
    };
